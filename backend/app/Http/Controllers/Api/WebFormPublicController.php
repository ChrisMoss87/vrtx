...
class WebFormPublicController extends Controller
{
    ...
    {
        $form = $this->webFormService->getFormBySlug($slug);
        ...
        // Record view
        $this->submissionService->recordFormView($form);

        return response()->json([
            'data' => $this->transformPublicForm($form),
        ]);
    }
    ...
    public function submit(Request $request, string $slug): JsonResponse
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => $form->thank_you_config['message'] ?? 'Thank you for your submission!',
    ...
    public function embedScript(string $slug): Response
    {
        ...
        $formData = json_encode($this->transformPublicForm($form));
        $submitUrl = url("/forms/{$slug}/submit");

        $js = $this->generateEmbedScript($form, $formData, $submitUrl);

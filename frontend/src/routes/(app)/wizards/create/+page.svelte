<script lang="ts">
	...
	import { createWizard, type WizardStep, type WizardSettings } from '$lib/api/wizards';
	import { getActiveModules, getModuleFields, type Module, type Field } from '$lib/api/modules';
	...
	let selectedStepId = $state<string>(steps[0].id);

	const selectedStep = $derived(steps.find((s) => s.id === selectedStepId) || steps[0]);
	...
	function updateStep(stepId: string, updates: Partial<StepConfig>) {
	...
	function assignFieldToStep(fieldId: string) {
		...
		steps = steps.map((s) =>
			s.id === selectedStepId ? { ...s, fields: [...s.fields, fieldId] } : s
		);
	}
	...
	function removeFieldFromStep(fieldId: string) {
		if (!selectedStep) return;
		steps = steps.map((s) =>
			s.id === selectedStepId ? { ...s, fields: s.fields.filter((f) => f !== fieldId) } : s
		);
	}
	...
	async function handleSave() {
		...
		try {
			...
			goto(`/wizards/${wizard.id}/edit`);
		} catch (error) {
			console.error('Failed to create wizard:', error);
			toast.error('Failed to create wizard');
		} finally {
			saving = false;
		}
	}
...
<div class="container mx-auto py-8">
	...
	<div class="mb-8">
		<div class="flex items-center gap-4">
			...
			<div class="flex-1">
				<h1 class="text-3xl font-bold">Create Wizard</h1>
				<p class="mt-1 text-muted-foreground">Build a multi-step form wizard</p>
			</div>
						...
						<Card.Content class="space-y-6">
							...
							<div class="grid gap-4 md:grid-cols-2">
								<div class="space-y-2">
									...
									<Input
										...
										oninput={(e) => updateStep(selectedStep.id, { title: e.currentTarget.value })}
								...
								<div class="space-y-2">
									...
									<Select.Root
										...
										onValueChange={(v) => updateStep(selectedStep.id, { type: v as StepConfig['type'] })}
									>
										<Select.Trigger>
							...
							<div class="space-y-2">
								...
								<Textarea
									...
									oninput={(e) =>
										updateStep(selectedStep.id, { description: e.currentTarget.value })}
							...
							<div class="flex items-center gap-2">
								<Switch
									...
									onCheckedChange={(checked) => updateStep(selectedStep.id, { can_skip: checked })}
								/>

<script lang="ts">
			...
			if (module?.blocks) {
				for (const block of module.blocks) {
					for (const field of block.fields) {
						formData[field.api_name] = field.default_value ?? getDefaultValueForType(field.type);
					}
				}
			}
		} catch (err) {
	...
	async function handleSubmit() {
		...
		try {
			const record = await recordsApi.create(moduleApiName, formData);
			toast.success(`${module?.singular_name || 'Record'} created successfully`);
			goto(`/records/${moduleApiName}`);
		} catch (err: any) {
			const message = err.response?.data?.error || err.message || 'Failed to create record';
			toast.error(message);
		...
		<div class="space-y-6">
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-4">
					...
					<div>
						<h1 class="text-2xl font-bold">Create {module.singular_name}</h1>
						<p class="mt-1 text-muted-foreground">Fill in the details below</p>
					</div>
				</div>

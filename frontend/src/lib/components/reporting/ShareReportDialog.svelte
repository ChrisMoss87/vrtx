<script lang="ts">
	...
	import { reportsApi, type ReportShare, type ShareRequest } from '$lib/api/reports';
	...
	let { reportId, open = $bindable(false), onOpenChange }: Props = $props();

	let shares = $state<ReportShare[]>([]);
	...
	async function handleShare() {
		...
		saving = true;
		try {
			const shareRequests: ShareRequest[] = selectedIds.map((id) => ({
				type: 'user',
				id,
				permission
			}));

			await reportsApi.share(reportId, shareRequests);
			toast.success('Report shared successfully');

			// Reload shares
			shares = await reportsApi.getShares(reportId);
			selectedIds = [];
			search = '';
		} catch (error) {
	...
	async function handleRemoveShare(share: ReportShare) {
		try {
			const targetId = share.user!.id;
			await reportsApi.unshare(reportId, 'user', targetId);
...
<Dialog.Root bind:open onOpenChange={handleOpenChange}>
	<Dialog.Content class="max-w-lg max-h-[80vh] flex flex-col">
		<Dialog.Header>
			...
			<Dialog.Description>Share this report with specific users</Dialog.Description>
		</Dialog.Header>

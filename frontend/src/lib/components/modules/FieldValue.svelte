...
{:else if field.type === 'email'}
	<a href={`mailto:${value}`} class="inline-flex items-center gap-1 text-primary hover:underline">
		<Mail class="h-3 w-3" />
		{value}
	</a>
{:else if field.type === 'phone'}
	<a href={`tel:${value}`} class="inline-flex items-center gap-1 text-primary hover:underline">
		<Phone class="h-3 w-3" />
		{value}
	</a>
...
{:else if field.type === 'textarea' || field.type === 'rich_text'}
	<div class="rounded-md bg-muted p-3 text-sm whitespace-pre-wrap">
		{value}
	</div>
{:else}
	{String(value)}
